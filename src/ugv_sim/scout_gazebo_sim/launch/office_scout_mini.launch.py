...
def generate_launch_description():
    ...
    ld.add_action(TimerAction(period=6.0, actions=[spawner_js]))
    ld.add_action(TimerAction(period=7.0, actions=[spawner_grip]))
    ld.add_action(TimerAction(period=7.7, actions=[spawner_height]))
    ld.add_action(TimerAction(period=8.5, actions=[ekf_node]))
    ld.add_action(TimerAction(period=9.0, actions=[nav2_bringup]))
    ld.add_action(TimerAction(period=11.0, actions=[rviz_node]))
    ld.add_action(TimerAction(period=12.0, actions=[dock_navigator]))
