...
class MapVisualizer {
    ...
    handleAMCLPose(message) {
        ...
        // 좌표 변환: 동쪽 정렬 → 북쪽 정렬
        const mapX = -(message.pose.pose.position.y) - this.correctionValue.y;
        const mapY = -(message.pose.pose.position.x) - this.correctionValue.x;

        this.robotX = mapX;
        this.robotY = mapY;

        // 오일러 변환 후 북쪽 기준으로 각도 정렬 (-90도 추가)
